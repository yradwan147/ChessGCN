...
sbatch --time=30:00:00 -J a1_baseline slurm/run_experiment.sh a1_baseline $H $HEADS $BLK $SIMS 3 $BASE

# a2: Attention pooling only
echo "  a2: Attention pooling"
sbatch --time=30:00:00 -J a2_attnpool slurm/run_experiment.sh a2_attnpool $H $HEADS $BLK $SIMS 3 $BASE --attn-pool

# a3: Moves-left head only
echo "  a3: Moves-left head"
sbatch --time=30:00:00 -J a3_movesleft slurm/run_experiment.sh a3_movesleft $H $HEADS $BLK $SIMS 3 $BASE --moves-left-head

# a4: Score distribution head (128 bins)
echo "  a4: Score distribution (128 bins)"
sbatch --time=30:00:00 -J a4_scoredist slurm/run_experiment.sh a4_scoredist $H $HEADS $BLK $SIMS 3 $BASE --score-dist-bins 128

# a5: Attention pool + moves-left (no score dist)
echo "  a5: Attn pool + moves-left"
sbatch --time=30:00:00 -J a5_arch_combo slurm/run_experiment.sh a5_arch_combo $H $HEADS $BLK $SIMS 3 $BASE --attn-pool --moves-left-head

# a6: Everything (all 3 arch changes)
echo "  a6: All Phase 2 changes"
sbatch --time=30:00:00 -J a6_all_p2 slurm/run_experiment.sh a6_all_p2 $H $HEADS $BLK $SIMS 3 $BASE --attn-pool --moves-left-head --score-dist-bins 128
